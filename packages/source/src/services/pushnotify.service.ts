...
import Notification from 'node-pushnotifications';
...
} from '../common/dto/push-notify.dto';
import { DEVICE, MODE, SILENT, STATUS, TYPE } from '../common/enum/common.enum';
import {
  DynamicKeyAnyDto,
  DynamicKeyMixDto,
  StandardPromiseDto,
...
export class PushNotifyService {
  ...
  async processPushNotification(
    ...
    try {
      ...
      const pushTemplate: PushTemplateDto =
        await this.getPushNotifyByCode(code);
