...
import { FailureCaseDto, SuccessCaseDto } from '../common/dto/external-api.dto';
import {
  DynamicKeyStrDto,
  StandardCallbackDto,
  StandardResultDto,
...
export class HttpService {
    ...
    try {
      if ('async' in options && options.async === true) {
        axios
          ...
          .then((response) => {
            (response.data as NodeJS.ReadableStream).pipe(
              fse.createWriteStream(loc),
            );
            if (options?.callback) {
          ...
          axios
            ...
            .then((response) => {
              ...
              if (loc) {
                ...
                (response.data as NodeJS.ReadableStream).pipe(b);
                return finished(b);
                // b.on("finish", resolve);
  ...
  //     key = `${key.substr(0, 0)}${key.substr(0 + len)}`;
  //   }
