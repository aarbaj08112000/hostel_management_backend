...
export class CitGeneralLibrary extends GeneralLibrary {
  ...
  private readonly lookupEntityRepo: Repository<LookupEntity>;
  ...
  async getAdminNameDb(id, entity_name) {
    try {
      if (id > 0) {
        ...
        if (entity_name == 'customer') {
          const user_data = await this.dataSource.query(
            `SELECT CONCAT(firstName, ' ', lastName) AS name FROM mod_customer WHERE id = ?`,
            [id],
          );
        } else {
          user_data = await this.dataSource.query(
            'SELECT name FROM mod_admin WHERE id = ?',
            [id],
          );
        }
  ...
  async getCarName(id: number): Promise<string | null> {
    if (id > 0) {
      const lookup = await this.lookupEntityRepo
        .createQueryBuilder('lookup')
        .select(
          "JSON_UNQUOTE(JSON_EXTRACT(lookup.entityJson, '$.name'))",
          'name',
        )
        .where('lookup.entityId = :id', { id })
  ...
  async getAdminData() {
    ...
    try {
      ...
      const data = await Promise.all(
        results.hits.map(async (hit) => hit._source['email']),
      );
  ...
  async addActivityLogs(inputParams: ActivityLogAddDto) {
    try {
  ...
  async getActivityCode(module_name: string, module_api: string) {
    let code: string = '';
    switch (module_api) {
      case 'add':
  ...
  async addActivity(moduleName, moduleAPI, added_by, value_json, request_id) {
    ...
    activityParams.added_by_name = await this.getAdminName(
      added_by,
      activityParams.added_by_type,
      activityParams.module_code,
    );
  ...
  async deleteAwsFile(filePath: string) {
    let options: any = {};
  ...
  verifyAdminResetPassword(inputParams, reqObj) {
    ...
    let oldPasswordsLimit = Number(
      this.configService.get('app.admin_password_history'),
    );
  ...
  getAutocompleteWhere(queryObject, inputParams, extraConfig) {
    ...
    if (extraConfig.table_name === 'mod_group_master') {
      const restrictedGroups = this.configService.get(
        'app.restrict_admin_groups',
      );
      if (!restrictedGroups.includes(inputParams.group_code)) {
        queryObject.andWhere(
          `${extraConfig.table_alias}.groupCode NOT IN (:...restrictedGroups)`,
  ...
  getAdminWhereCriteria(
    ...
    const restrictAdminGroups = this.configService.get(
      'app.restrict_admin_groups',
    );
    if (actionType) {
      switch (actionType) {
  ...
  getGroupWhereCriteria(type = '', inputParams, queryObject, requestObj) {
    let whereCond: any = '';
    const defaultAdminGroups = this.configService.get(
      'app.default_admin_groups',
    );
    const restrictAdminGroups = this.configService.get(
      'app.restrict_admin_groups',
    );
  ...
  stringifyJson(value: any = '', name: any = '', obj: any = '') {
    return JSON.stringify(name.preferences);
  }

  updateExportOpts(inputParams) {
    if (
      inputParams.export_mode == 'all' ||
  ...
  numberFormat(
  ...
  ) {
    {
      ...
      const symbolRegex = /[^\d.,]/g; // Regular expressioon to match non-digit, non-comma, non-decimal characters
      let extractedValue = formattedValue.replace(symbolRegex, ''); // Remove non-digit, non-comma, non-decimal characters
      if (is_currency == 'yes') {
  ...
  createElasticSearchQuery(params) {
    ...
    let mustClauses = Object.entries(cleanedQueryParams)
      .filter(
        ([key]) => key !== 'address' && key !== 'distance' && key !== 'price',
      )
      .map(([key, value]) =>
        Array.isArray(value)
          ? { terms: { [key]: value } }
    ...
    let sortClauses = Object.entries(cleanedSortParams).map(
      ([key, { prop, dir }]) => ({
        [prop]: { order: dir },
      }),
    );
    let boostClauses = Object.entries(boostParams).flatMap(([field, items]) =>
      items.map(({ key, boost }) => ({
        term: {
          [field]: {
            value: key,
    ...
    if (Object.keys(geoFilter).length > 0) query.bool['filter'] = geoFilter;
    if (Object.keys(priceFilter).length > 0) query.bool['filter'] = priceFilter;
    if (Object.keys(boostClauses).length > 0)
      query.bool['should'] = boostClauses;
    let result = { query };
    if (sortClauses.length > 0) result['sort'] = sortClauses;
    return result;
  }
  ...
  createTimeSlots(startDate: string, endDate: string, durationMinutes: number) {
    const startTime = parse(startDate, 'yyyy-MM-dd HH:mm:ss', new Date());
    const endTime = parse(endDate, 'yyyy-MM-dd HH:mm:ss', new Date());
    const timeSlots = [];
    ...
    while (isBefore(currentStartTime, endTime)) {
      ...
      if (
        isBefore(currentEndTime, endTime) ||
        currentEndTime.getTime() === endTime.getTime()
      ) {
        const slotTime = `${format(currentStartTime, 'HH:mm:ss')} - ${format(currentEndTime, 'HH:mm:ss')}`;
        const displayTime = `${format(currentStartTime, 'HH:mm')} - ${format(currentEndTime, 'HH:mm')}`;
  ...
  async applyDynamicSelectsFromLookup(
    queryObject,
    selFields: Record<string, string>,
    tableAlias: string,
  ) {
    if (!selFields || typeof selFields !== 'object') return;
  ...
  async findOneByDynamicFields(
    alias: string,
    filters: Record<string, any>,
    jsonColumn: string = 'entityJson',
  ): Promise<any> {
    ...
    for (const [key, val] of Object.entries(filters)) {
      ...
      const isJsonField =
        !(await this.lookupEntityRepo.metadata.findColumnWithPropertyName(key));
      if (isJsonField) {
        queryBuilder.andWhere(
          `JSON_UNQUOTE(JSON_EXTRACT(${alias}.${jsonColumn}, '$.${key}')) = :${paramKey}`,
          { [paramKey]: val },
        );
      } else {
